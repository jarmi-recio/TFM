...
\vspace{1cm}
%TODO_DONE: debes traducirlo al Castellano
\begin{algorithm}[H]
  \KwData{p(T): distribución de tareas}
  \KwData{$\alpha$, $\beta$ : tamaños de los pasos de actualización de hiperparámetros}
  Inicialización aleatoria de $\theta$\;
  \While{not done}{
  Muestreo de un \emph{batch} de tareas $T_{i}$ $\sim$ $p(T)$\;
  \For{all $Ti$}{
    Evaluar $\nabla_{\theta}$ $L_{T_{i}}(f_{\theta})$ respecto a las K muestras\;
    Realizar la adaptación de parámetros mediante el descenso de gradiente: $\theta_{i}'$ = $\theta$ - $\alpha$ $\nabla_{\theta}$ $L_{Ti}(f_{\theta})$\;
  }
  Actualizar $\theta$ ← $\theta$ - $\beta$ $\nabla_{\theta}$ $\sum_{Ti \sim p(T)}$ $L_{Ti}(f_{\theta_{i}'})$
  }
...
%TODO_DONE: Gracias a ti Jarmi. Esto lo dejamos fuera de la memoria, pues para eso está la sección de agradecimientos.
