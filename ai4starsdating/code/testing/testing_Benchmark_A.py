...
# load model for further analysis
models = load('../../models/models_Benchmark_A.joblib')
...
for name, model in models.items():
    ...
    df_final_test = pd.DataFrame(list(zip(y_test_new, y_pred_model, reg_error)),
                                 columns=['y_test', 'y_pred_model', 'reg_error'])
    ...
    ax2.fill_between(n, X_test_limits_df['low_age'], X_test_limits_df['high_age'], color="gray", alpha=0.5,
                     label="Margin")
