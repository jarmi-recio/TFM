...
# perform train test split without shuffling (test 20%)
...
plt.savefig('../../pics/training_testing/Benchmark_B1_MAE_hist.pdf')
...
for name, model in models.items():
	...
	ax1.plot(range(4, 15), range(4, 15), color='black')
	...
	plt.xticks(np.arange(4, 15, 1))
	plt.xlabel("Age (Gyr)")
	plt.ylabel("Prediction (Gyr)")
	plt.legend(loc='upper left')
	plt.savefig('../../pics/training_testing/Benchmark_B1_'+name+'_fig1.pdf')
	...
	plt.legend()
	plt.grid(True)
	plt.savefig('../../pics/training_testing/Benchmark_B1_'+name+'_fig2.pdf')
...
# saving models
dump(models,'../../models/models_Benchmark_B1_originals.joblib')
